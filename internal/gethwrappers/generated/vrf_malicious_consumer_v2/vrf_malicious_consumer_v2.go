...
func DeployVRFMaliciousConsumerV2(auth *bind.TransactOpts, backend bind.ContractBackend, vrfCoordinator common.Address, phb common.Address) (common.Address, *types.Transaction, *VRFMaliciousConsumerV2, error) {
	...
	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(VRFMaliciousConsumerV2Bin), backend, vrfCoordinator, phb)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
