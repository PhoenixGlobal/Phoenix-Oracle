...
func Router(app phoenix.Application) *gin.Engine {
	engine := gin.New()
	store := app.GetStore()
	config := store.Config
	...
	api := engine.Group(
		...
		sessions.Sessions(web.SessionName, sessionStore),
	)
...
func metricRoutes(app phoenix.Application, r *gin.RouterGroup) {
	group := r.Group("/debug", web.RequireAuth(app.GetStore(), web.AuthenticateBySession))
...
func sessionRoutes(app phoenix.Application, r *gin.RouterGroup) {
	...
	auth := r.Group("/", web.RequireAuth(app.GetStore(), web.AuthenticateBySession))
	auth.DELETE("/sessions", sc.Destroy)
}

func healthRoutes(app phoenix.Application, r *gin.RouterGroup) {
	hc := HealthController{app}
	r.GET("/readyz", hc.Readyz)
	r.GET("/health", hc.Health)
}

func v2Routes(app phoenix.Application, r *gin.RouterGroup) {
	...
	authv2 := r.Group("/v2", web.RequireAuth(app.GetStore(), web.AuthenticateByToken, web.AuthenticateBySession))
	{
		...
		authv2.GET("/external_initiators", web.PaginatedRequest(eia.Index))
		authv2.POST("/external_initiators", eia.Create)
		authv2.DELETE("/external_initiators/:Name", eia.Destroy)

		bt := BridgeTypesController{app}
		authv2.GET("/bridge_types", web.PaginatedRequest(bt.Index))
		...
		authv2.GET("/tx_attempts", web.PaginatedRequest(tas.Index))

		txs := TransactionsController{app}
		authv2.GET("/transactions", web.PaginatedRequest(txs.Index))
		...
		authv2.GET("/jobs", web.PaginatedRequest(jc.Index))
		...
		authv2.GET("/pipeline/runs", web.PaginatedRequest(prc.Index))
		authv2.GET("/jobs/:ID/runs", web.PaginatedRequest(prc.Index))
		...
		authv2.GET("/chains/evm", web.PaginatedRequest(chc.Index))
		authv2.POST("/chains/evm", chc.Create)
		authv2.DELETE("/chains/evm/:ID", chc.Delete)

		nc := NodesController{app}
		authv2.GET("/nodes", web.PaginatedRequest(nc.Index))
		authv2.GET("/chains/evm/:ID/nodes", web.PaginatedRequest(nc.Index))
		authv2.POST("/nodes", nc.Create)
		authv2.DELETE("/nodes/:ID", nc.Delete)
	}

	ping := PingController{app}
	userOrEI := r.Group("/v2", web.RequireAuth(app.GetStore(),
		web.AuthenticateExternalInitiator,
		web.AuthenticateByToken,
		web.AuthenticateBySession,
	))
...
func guiAssetRoutes(box packr.Box, engine *gin.Engine, config config.GeneralConfig) {
	...
	assetsRouterHandlers = append(
		...
		web.ServeGzippedAssets("/assets", &web.BoxFileSystem{Box: box}),
	)
