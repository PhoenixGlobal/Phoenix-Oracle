...
import (
	...
	evmtypes "PhoenixOracle/core/chain/evm/types"
...
func NewBulletproofTxManager(db *gorm.DB, ethClient ethereum.Client, config Config, keyStore KeyStore,
...
func newAttempt(ethClient ethereum.Client, ks KeyStore, chainID *big.Int, etx EthTx, gasPrice *big.Int, gasLimit uint64) (EthTxAttempt, error) {
...
func sendTransaction(ctx context.Context, ethClient ethereum.Client, a EthTxAttempt, e EthTx, logger *logger.Logger) *ethereum.SendError {
	signedTx, err := a.GetSignedTx()
	if err != nil {
		return ethereum.NewFatalSendError(err)
	}
...
func CheckEthTxQueueCapacity(db *gorm.DB, fromAddress common.Address, maxQueuedTransactions uint64) (err error) {
	...
	if err != nil {
		err = errors.Wrap(err, "txmanager.CheckEthTxQueueCapacity query failed")
		return
	}
