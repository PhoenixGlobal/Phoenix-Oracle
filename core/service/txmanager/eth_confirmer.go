...
func NewEthConfirmer(db *gorm.DB, ethClient ethereum.Client, config Config, keystore KeyStore, advisoryLocker postgres.AdvisoryLocker, keys []ethkey.KeyV2, estimator gas.Estimator, resumeCallback func(id uuid.UUID, value interface{}) error, logger *logger.Logger) *EthConfirmer {
...
func (ec *EthConfirmer) CheckForReceipts(ctx context.Context, blockNum int64) error {
	...
	for from, attempts := range attemptsByAddress {
		ctxInner, cancel := ethereum.DefaultQueryCtx(ctx)
...
RETURNING e0.id, e0.nonce, e0.from_address`, ErrCouldNotGetReceipt, cutoff)
	...
	for rows.Next() {
		...
		ec.logger.Errorw(fmt.Sprintf("EthConfirmer: eth_tx with ID %v expired without ever getting a receipt for any of our attempts. "+
			...
			"This can happen if there is another instance of phoenix running that is using the same private key, or if "+
			"an external wallet has been used to send a transaction from account %s with nonce %v."+
			" Please note that Phoenix requires exclusive ownership of it's private keys and sharing keys across multiple"+
			" phoenix instances, or using the phoenix keys with an external wallet is NOT SUPPORTED and WILL lead to missed transactions",
...
func (ec *EthConfirmer) attemptForRebroadcast(ctx context.Context, etx EthTx) (attempt EthTxAttempt, err error) {
	...
	if len(etx.EthTxAttempts) > 0 {
		...
		ec.logger.Errorf("invariant violation: EthTx %v was unconfirmed but didn't have any attempts. "+
			...
			"This is a bug! Please report to https://PhoenixOracle/issues", etx.ID)
...
func (ec *EthConfirmer) handleInProgressAttempt(ctx context.Context, etx EthTx, attempt EthTxAttempt, blockHeight int64) error {
	...
	if sendError.IsReplacementUnderpriced() {
		ec.logger.Errorw(fmt.Sprintf("EthConfirmer: replacement transaction underpriced at %v wei for eth_tx %v. "+
			...
			"Please note that using your node's private keys outside of the phoenix node is NOT SUPPORTED and can lead to missed transactions.",
	...
	if sendError.IsInsufficientEth() {
		ec.logger.Errorw(fmt.Sprintf("EthConfirmer: EthTxAttempt %v (hash 0x%x) at gas price (%s Wei) was rejected due to insufficient eth. "+
			...
			"ACTION REQUIRED: Phoenix wallet with address 0x%x is OUT OF FUNDS",
