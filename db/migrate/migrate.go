...
import (
	...
	"PhoenixOracle/db/migrate/migrations" // Invoke init() functions within migrations pkg.
...
func ensureMigrated(db *sql.DB) {
	...
	if !found {
		panic("Database state is too old. Need to migrate to phoenix version 0.9.10 first before upgrading to this version. This upgrade is NOT REVERSIBLE, so it is STRONGLY RECOMMENDED that you take a database backup before continuing.")
	}
