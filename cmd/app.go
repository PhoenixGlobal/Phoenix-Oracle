...
func NewApp(client *Client) *cli.App {
	app := cli.NewApp()
	app.Usage = "CLI for PhoenixOracle"
	...
	app.Commands = removeHidden([]cli.Command{
		...
		{
			Name:  "keys",
			Usage: "Commands for managing various types of keys used by the Oracle node",
			Subcommands: []cli.Command{
				{
					Name:  "eth",
					...
					Subcommands: cli.Commands{
						...
						{
							Name:   "list",
							Usage:  "List available Ethereum accounts with their ETH & PHB balances, nonces, and other metadata",
		...
		},
		{
			...
			Description: "Commands can only be run from on the same machine as the Oracle node.",
			Subcommands: []cli.Command{
				{
					Name:        "deleteuser",
				...
				},
				{
					...
					Flags: []cli.Flag{
						...
						cli.StringFlag{
							Name:  "vrfpassword, vp",
							Usage: "textfile holding the password for the vrf keys; enables phoenix VRF oracle",
						},
					},
					Usage:  "Run the phoenix node",
